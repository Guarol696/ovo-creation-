...
import { PracticalSection } from "./practical-section";
...
export function TravelPlanView({ plan }: { plan: TravelPlan }) {
  return (
    <div className="relative isolate overflow-x-clip bg-night-950 text-white">
      ...
      <BudgetSection budget={plan.estimatedBudget} request={plan.request} travelers={plan.travelers.total} />
      <PracticalSection plan={plan} />
      <ResultCta plan={plan} />
    </div>
  );
}
